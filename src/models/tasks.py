...
class ExtractedTask(BaseModel):
    ...
    estimated_effort: Optional[EffortSize] = Field(None, description="Estimated effort size")
    ...
    # Source tracking
    source_document: Optional[str] = Field(None, description="Source document type (TDD/PRD)")
    ...
    def validate_dependencies(cls, v, values):
        ...
        if 'title' in values and values['title'] in v:
            raise ValueError("Task cannot depend on itself")
        return v
    
    @validator('priority')
    def adjust_critical_priority(cls, v, values):
        ...
        if 'title' in values and 'description' in values:
            ...
            content = f"{values['title']} {values['description']}".lower()
...
class TaskBatch(BaseModel):
    ...
    tasks: List[ExtractedTask] = Field(..., min_items=1, description="List of tasks")
